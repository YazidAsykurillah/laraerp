<?php
    ...
    {
        if($sort == 'y'){
            $list_transaction_k = \DB::table('transaction_chart_accounts')->where([['sub_chart_account_id',$key]])->where([['type','keluar']])->where('created_at','like',$date.'%')->where('description','!=','SALDO AWAL')->sum('amount');
            $list_transaction_d = \DB::table('transaction_chart_accounts')->where([['sub_chart_account_id',$key]])->where([['type','masuk']])->where('created_at','like',$date.'%')->where('description','!=','SALDO AWAL')->sum('amount');
            if(count($list_transaction_k) == 0){
                 return $list_transaction_d;
             }
                return $list_transaction_d-$list_transaction_k;
        }elseif ($sort == 'm') {
            $list_transaction_k = \DB::table('transaction_chart_accounts')->where([['sub_chart_account_id',$key]])->where([['type','keluar']])->whereBetween('created_at',[$date,$end])->where('description','!=','SALDO AWAL')->sum('amount');
            $list_transaction_d = \DB::table('transaction_chart_accounts')->where([['sub_chart_account_id',$key]])->where([['type','masuk']])->whereBetween('created_at',[$date,$end])->where('description','!=','SALDO AWAL')->sum('amount');
            if(count($list_transaction_k) == 0){
                 return $list_transaction_d;
             }
                return $list_transaction_d-$list_transaction_k;
    ...
    function list_transaction_modal($key,$date,$sort,$end,$description)
    {
        if($sort == 'y')
        {
            $list_transaction_d = \DB::table('transaction_chart_accounts')->where([['sub_chart_account_id',$key]])->where([['type','masuk']])->where('created_at','like',$date.'%')->where([['description',$description]])->sum('amount');
