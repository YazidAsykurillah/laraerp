...
@section('content')
  {!! Form::model($adjustment,['route'=>['product-adjustment.update', $adjustment], 'class'=>'form-horizontal','id'=>'form-edit-bank', 'method'=>'put']) !!}
  <div class="row">
    <div class="col-lg-12">
      <div class="box" style="box-shadow:0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);border-top:none">
        ...
        <div class="box-body">
          <div class="form-group{{ $errors->has('code') ? ' has-error' : '' }}">
            {!! Form::label('code', 'Adjust No', ['class'=>'col-sm-2 control-label']) !!}
            <div class="col-sm-4">
              {!! Form::text('code',null,['class'=>'form-control', 'placeholder'=>'Adjust no of the products', 'id'=>'code']) !!}
              @if ($errors->has('code'))
            ...
            <table class="table table-striped table-hover" id="table-selected-products">
                ...
                <tbody>
                    @foreach($adjustment->product_adjustment as $pa)
                       <tr id="tr_product_{{$pa->product_id}}">
                           <td>
                               {{$pa->product->main_product->family->name}}
                               <input type="hidden" name="product_id[]" value="{{$pa->product_id}}" />
                               <input type="hidden" name="family_name[]" value="{{$pa->product->main_product->family->name}}" />
                           </td>
                           <td>
                               {{$pa->product->name}}
                               <button type="button" class="btn btn-info btn-xs btn-view-products" data-id="{{$pa->product_id}}" data-text="{{$pa->product->name}}" data-stock="{{$pa->product->stock}}" title="View Stock">
          ...
          <div class="form-group">
            {!! Form::hidden('adjustment_id',$adjustment->id, ['class'=>'col-sm-3 control-label']) !!}
            <div class="col-sm-9">
...
@section('additional_scripts')
  ...
  <script type="text/javascript">
    ...
    @foreach($adjustment->product_adjustment as $product)
      selected.push({{$product->product->id}});
    @endforeach
